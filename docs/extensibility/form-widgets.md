...
- **required** - a boolean which specifies if a field is required. The default value is taken from the CustomResourceDefintion (CRD); if it doesn't exist in the CRD, then it defaults to `false`.
