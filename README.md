...
### Connectivity issues with Busola against k3d cluster
...
Busola run in Docker in macOS can't connect to the k3d cluster and there are errors in the container log, like:
```
...
Busola run in a Docker container with env variable `DOCKER_DESKTOP_CLUSTER=true` replaces the IP `0.0.0.0` in the API Server URL with a `host.docker.internal`. Kubernetes is not aware of that host name, so its API Server doesn't have it in the SSL certificate, which results in the above error.

Furthermore, this behavior has changed in the recent [k3d](https://k3d.io/) versions, which is a result of [this fix](https://github.com/k3s-io/k3s/commit/aa76942d0fcb23dd02c25aa7a0dfb96b6b915fa5) for [this security issue](https://github.com/k3s-io/k3s/security/advisories/GHSA-m4hf-6vgr-75r2).

Clusters installed by the [k3d](https://k3d.io/) use a [listener](https://github.com/rancher/dynamiclistener) that grabs [SNI](https://en.wikipedia.org/wiki/Server_Name_Indication) host names from requests sent to the API server. If a new host name is requested then the SSL certificate is regenerated and the new host name is added to the list of Subject Alternative Names. Unfortunately, the security fix limits this mechanism only to 'expected' host names, like those related to Kubernetes nodes. This makes it useless for `host.docker.internal` case.

#### Remedy

Provide `host.docker.internal` host name upfront during [k3d](https://k3d.io/) cluster creation:

```
k3d cluster create kyma --k3s-arg '--tls-san=host.docker.internal@server:*'
```

Cluster created in such a way has the `host.docker.internal` set as Subject Alternative Name in the SSL Certificate of the API Server since the very beginning.