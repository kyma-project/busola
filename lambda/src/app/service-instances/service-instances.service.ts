...
export class ServiceInstancesService {
  ...
  public getServiceInstances(namespace: string, token: string): Observable<ServiceInstanceList> {
    const httpOptions = this.getHTTPOptions(token);
    const url = `${AppConfig.serviceCatalogApiUrl}/namespaces/${namespace}/serviceinstances`;
    return this.http.get<ServiceInstanceList>(url, httpOptions);
  }

  public getServiceInstance(name: string, namespace: string, token: string): Observable<ServiceInstance> {
    const httpOptions = this.getHTTPOptions(token);
    const url = `${AppConfig.serviceCatalogApiUrl}/namespaces/${namespace}/serviceinstances/${name}`;
