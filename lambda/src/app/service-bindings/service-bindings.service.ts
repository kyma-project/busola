...
import { ServiceBinding, IServiceBinding, IServiceBindingSpec } from '../shared/datamodel/k8s/service-binding';
...
export class ServiceBindingsService {
  ...
  public getServiceBinding(name: string, namespace: string, token: string): Observable<ServiceBinding> {
    const httpOptions = this.getHTTPOptions(token);
    const url = `${AppConfig.serviceCatalogApiUrl}/namespaces/${namespace}/servicebindings/${name}`;
    return this.http.get<ServiceBinding>(url, httpOptions);
  }

  public getServiceBindings(namespace: string, token: string): Observable<ServiceBindingList> {
    const httpOptions = this.getHTTPOptions(token);
    const url = `${AppConfig.serviceCatalogApiUrl}/namespaces/${namespace}/servicebindings`;
    return this.http.get<ServiceBindingList>(url, httpOptions);
  }

  public createServiceBinding(serviceBinding: ServiceBinding, token: string): Observable<ServiceBinding> {
    const httpOptions = this.getHTTPOptions(token);
    const url = `${AppConfig.serviceCatalogApiUrl}/namespaces/${serviceBinding.metadata.namespace}/servicebindings`;
    return this.http.post<ServiceBinding>(url, serviceBinding, httpOptions);
  }

  public deleteServiceBinding(name: string, namespace: string, token: string): Observable<ServiceBinding> {
    const httpOptions = this.getHTTPOptions(token);
    const url = `${AppConfig.serviceCatalogApiUrl}/namespaces/${namespace}/servicebindings/${name}`;
