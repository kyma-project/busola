...
(function() {
  ...
  (function() {
    ...
    var PATCH_REASON =
      'Use iconv-lite instead of iconv, helpful for webpack bundling';
    console.log('patching `%s`(%s) module', PATCH_MODULE, PATCH_VERSION);
    var pathToModule = path.dirname(
      require.resolve(PATCH_MODULE + '/package.json'),
    );
    var pathToModulePackage = path.join(pathToModule, 'package.json');
    var pathToModulePatchedFile1 = path.join(
      pathToModule,
      'lib/iconv-loader.js',
    );
    ...
    if (fs.existsSync(pathToModulePatchedFile1)) {
      ...
      console.error(
        'patching `%s` failed because the file does not exist in',
        PATCH_MODULE,
        pathToModule,
      );
      process.exit(1);
