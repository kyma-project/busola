...
async function fetchNamespacedResource(context: CommandPaletteContext) {
  const { fetch, namespace, tokens, namespaceNodes, updateResourceCache } =
    context;
    ...
    .map((namespaceNode) => {
      return {
        ...
        onActivate: () => {
          const pathname = `/cluster/${encodeURIComponent(
            activeClusterName ?? '',
          )}/namespaces/${namespace}/${namespaceNode.pathSegment}`;
          navigate(pathname);
        },
...
function createSingleResult(
  ...
  const linkToList = {
    ...
    onActivate: () => {
      const pathname = `/cluster/${encodeURIComponent(
        activeClusterName ?? '',
      )}/namespaces/${namespace}/${matchedNode.pathSegment}`;
      navigate(pathname);
    },
