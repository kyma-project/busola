...
const PV_NFS_NAME = `test-pv-${Math.random().toString().substr(2, 8)}`;

const PV_CSI_NAME = `test-pv-${Math.random().toString().substr(2, 8)}`;
...
context('Test Persistent Volumes', () => {
  ...
  it('Check PV with NFS details', () => {
    cy.getMidColumn().contains('ReadWriteOnce').should('be.visible');

    cy.getMidColumn().contains('Events').should('be.visible');

    cy.getMidColumn().contains('Filesystem').should('be.visible');

    cy.getMidColumn().contains('Recycle').should('be.visible');

    cy.getMidColumn().contains('slow').should('be.visible');

    cy.getMidColumn().contains('Network File System').should('be.visible');

    cy.getMidColumn().contains('172.17.0.2').should('be.visible');

    cy.getMidColumn().contains('/tmp').should('be.visible');
  });
  ...
  it('Check PV with NFS details', () => {
    ...
    cy.getMidColumn().contains('ext4').should('be.visible');

    cy.getMidColumn()
      .contains('Container Storage Interface')
      .should('be.visible');

    cy.getMidColumn().contains('csi-driver.example.com').should('be.visible');

    cy.getMidColumn().contains('existingVolumeName').should('be.visible');
  });
